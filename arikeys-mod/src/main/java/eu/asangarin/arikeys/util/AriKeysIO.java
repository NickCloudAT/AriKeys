	...
	public static void load() {
		try {
			...
			for (AriKey ariKey : AriKeys.getKeybinds()) {
				...
				String rawKeybind = nbtCompound.contains(key) ? String.valueOf(nbtCompound.getString(key)) : defKey;

				// Strip Optional[...] if present
				if (rawKeybind.startsWith("Optional[") && rawKeybind.endsWith("]")) {
					rawKeybind = rawKeybind.substring(9, rawKeybind.length() - 1);
				}

				String keybind = MoreObjects.firstNonNull(rawKeybind, defKey);
